...
def _get_reserved_quantity(order_id: int, product_id: int, warehouse_id: int) -> int:
    """Возвращает зарезервированное количество товара для заказа на складе"""
    conn = get_conn()
    with conn.cursor() as cur:
        cur.execute("""
            SELECT COALESCE(SUM(quantity), 0)
            FROM inventory.reserves
            WHERE order_id = %s AND product_id = %s AND warehouse_id = %s
        """, (order_id, product_id, warehouse_id))
        return cur.fetchone()[0]


def _get_transfer_info(order_id: int, product_id: int, warehouse_id: int) -> tuple[
...
def _get_item_status(order_id: int, product_id: int, warehouse_id: int, order_status: str) -> tuple[
    str, Optional[str], Optional[datetime]]:
    """
    Вычисляет статус позиции заказа.
    Возвращает: (status_display, from_warehouse, arriving_at)
    """
    # Если заказ новый и не взят в обработку
    if order_status == 'new':
    ...
    # Проверяем резерв
    reserved = _get_reserved_quantity(order_id, product_id, warehouse_id)
    if reserved > 0:
        # Проверяем, не в пути ли товар
        transfer_status, from_warehouse, arriving_at = _get_transfer_info(order_id, product_id, warehouse_id)
        if transfer_status:
    ...
    # Проверяем, есть ли перемещения для этого товара
    transfer_status, from_warehouse, arriving_at = _get_transfer_info(order_id, product_id, warehouse_id)
    if transfer_status:
        if transfer_status == 'shipped':
            return f"в пути из {from_warehouse}", from_warehouse, arriving_at
...
def _render_order_card(order: Order) -> None:
    ...
    for item in items:
        ...
        # Дополняем статус информацией о прибытии
